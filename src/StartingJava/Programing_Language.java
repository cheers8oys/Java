...
public class Programing_Language {
   ...
   JDK안에 JRE가 포함되어 있고 JRE안에 JVM이 포함되어 있다.
    ...
    클래스 내부에 포함될 수 있는 내부 구성 요쇼 4가지 : 필드, 메서드, 생성자, 이너 클래스
    이들을 제외하고는 단 한줄도 올 수 없다.
    ...
    /*
    [main() 메서드]
    바이트 코드가 메서드 영역에 로딩되면 자바 가상 머신은 main()메서드부터 찾는다.
    가장 먼저 실행되는 메서드는 main()메서드이다***

    [소스 파일 컴파일과 바이트 코드 생성]

    소스 파일을 컴파일하면 소스 파일 내의 클래스의 숫자만큼 바이트 코드가 생성된다.
    ex. Car.java 소스 파일 내의 Class A, Class B, Class C가 존재한다면 A.class, B.class, C.class가 생성된다.

    근데 만약에 B클래스 안에 이너 클래스로 C클래스가 존재한다면 B$C.class의 형태로 바이트 코드가 생성 된다
